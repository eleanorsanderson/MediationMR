...
for(i in 1:nrow(params))
{
  ...
  o <- mediationpleiotropy(n,reps,params$total_effect[i], params$proportion_mediated[i], params$beta2[i])
  AA <- cbind(params[i,], o)
  resultsA <- rbind(resultsA, AA)
}


names <- c("total_effect", "proportion_mediated", "beta2", "beta1", "gamma", "mean_ols_b_e", "mean_mr_b_e", "mean_ols_s_e", "mean_ols_s_b", "mean_ols_s_eb_e", "mean_ols_s_eb_b", 
